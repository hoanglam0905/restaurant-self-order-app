...
class KitchenManagementView extends StatelessWidget {
  ...
  Widget _buildStatusFilter({required String selected, required VoidCallback onTap}) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            const Icon(Icons.filter_alt_rounded, size: 18, color: Color(0xFF4B5563)),
            const SizedBox(width: 8),
            Text(selected, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Color(0xFF334155))),
            const SizedBox(width: 6),
            const Icon(Icons.keyboard_arrow_down_rounded, size: 18, color: Color(0xFF334155)),
          ],
        ),
      ),
    );
  ...
  void _showSearchDialog(BuildContext context, KitchenController controller) {
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: const Text('Tìm kiếm đơn', style: TextStyle(fontWeight: FontWeight.bold)),
          content: TextField(
